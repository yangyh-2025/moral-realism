...
def fig6_leader_followers():
    ...
    for ax, (label, lf_data, color_palette) in zip(
        [ax1, ax2],
        [('S1-G1', data['leader_followers']['s1g1'], 'Set1'),
         ('Baseline', data['leader_followers']['baseline'], 'Set1')]
    ):
        leaders = sorted(lf_data.keys())
        cmap = plt.get_cmap('tab10', len(leaders))

        for i, leader in enumerate(leaders):
            vals = lf_data[leader]
            rounds = sorted(vals.keys())
            r_vals = [vals[r] for r in rounds]
            ax.plot(rounds, r_vals, color=cmap(i), linewidth=1.5, marker='o', markersize=2, label=leader)

        ax.set_ylabel('Follower Count')
        ax.set_title(f'{label}')
        ax.legend(loc='upper right', ncol=len(leaders), fontsize=8)
        ax.grid(True, alpha=0.3, color=COLORS['grid'])

    ax2.set_xlabel('Round')
    fig.suptitle('Figure 6: Leader Follower Counts Over Time', fontsize=14, y=1.01)
    fig.tight_layout()
