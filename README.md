...
系统定义了**34种**详细的行动类型，分为5大类：

| 类别 | 数量 | 说明 |
|------|------|------|
| 🛡️ **安全类行动** | 8种 | 军事部署、同盟、调解、演习、军备、安全保证、武器出口、不扩散承诺 |
| 💰 **经济类行动** | 9种 | 贸易合作、制裁、援助、自由贸易、关税调整、公共物品、供应链合作、金融支持、贸易争端 |
| ⚖️ **规范类行动** | 8种 | 规范提议、改革、组织改革、条约签署/退出、价值外交、道德评价、争端仲裁 |
| 🤝 **外交类行动** | 6种 | 访问、同盟、关系升级/降级、承认、峰会、联合声明、舆论引导 |
| 🚫 **特殊行动** | 1种 | 不采取行动 |

### 安全类行动 (Security Actions)

| 行动代码 | 中文名 | 英文名 | 详细描述 |
|---------|--------|--------|----------|
| `security_military` | 军事行动 | Military Action | 部署或使用军事力量用于安全目的 |
| `security_alliance` | 安全同盟 | Security Alliance | 建立、加强或维护安全同盟 |
| `security_mediation` | 安全调解 | Security Mediation | 在冲突各方之间进行调解 |
| `military_exercise` | 军事演习 | Military Exercise | 与盟友进行军事演习 |
| `military_escalation` | 军事升级 | Military Escalation | 增强军事能力或军备建设 |
| `security_guarantee` | 安全保证 | Security Guarantee | 向盟友提供安全承诺 |
| `weapons_export` | 武器出口 | Weapons Export | 出口武器或军事技术 |
| `non_proliferation_commit` | 不扩散承诺 | Non-proliferation Commit | 承诺不扩散核武器 |

### 经济类行动 (Economic Actions)

| 行动代码 | 中文名 | 英文名 | 详细描述 |
|---------|--------|--------|----------|
| `economic_trade` | 经济贸易 | Economic Trade | 开展贸易协定或经济合作 |
| `economic_sanction` | 经济制裁 | Economic Sanction | 实施、维持或解除经济制裁 |
| `economic_aid` | 经济援助 | Economic Aid | 提供经济援助或发展援助 |
| `free_trade_agreement` | 自由贸易协定 | Free Trade Agreement | 签署或谈判自由贸易协定 |
| `tariff_adjustment` | 关税调整 | Tariff Adjustment | 调整进出口关税 |
| `public_goods_provision` | 公共物品提供 | Public Goods Provision | 提供国际公共物品 |
| `supply_chain_coop` | 供应链合作 | Supply Chain Cooperation | 供应链合作 |
| `financial_support` | 金融支持 | Financial Support | 提供金融援助或贷款 |
| `trade_dispute` | 贸易争端 | Trade Dispute | 启动贸易争端程序 |

### 规范类行动 (Norm Actions)

| 行动代码 | 中文名 | 英文名 | 详细描述 |
|---------|--------|--------|----------|
| `norm_proposal` | 规范提议 | Norm Proposal | 提出新的国际规范或原则 |
| `norm_reform` | 规范改革 | Norm Reform | 提出改革现有国际规范 |
| `org_reform` | 组织改革 | Organization Reform | 提出国际组织改革 |
| `treaty_sign` | 条约签署 | Treaty Sign | 签署国际条约 |
| `treaty_withdraw` | 条约退出 | Treaty Withdraw | 退出国际条约 |
| `value_diplomacy` | 价值外交 | Value Diplomacy | 开展基于价值的外交 |
| `moral_judgement` | 道德评价 | Moral Judgement | 发出国际道德评价 |
| `dispute_arbitration` | 争端仲裁 | Dispute Arbitration | 启动国际争端仲裁 |

### 外交类行动 (Diplomatic Actions)

| 行动代码 | 中文名 | 英文名 | 详细描述 |
|---------|--------|--------|----------|
| `diplomatic_visit` | 外交访问 | Diplomatic Visit | 国事访问、外交沟通 |
| `diplomatic_alliance` | 外交同盟 | Diplomatic Alliance | 正式外交、战略同盟 |
| `alliance_upgrade` | 同盟升级 | Alliance Upgrade | 升级同盟关系承诺 |
| `alliance_downgrade` | 同盟降级 | Alliance Downgrade | 降级或减少同盟承诺 |
| `diplomatic_recognition` | 外交承认 | Diplomatic Recognition | 承认或撤回外交承认 |
| `summit_host` | 主办峰会 | Summit Host | 主办国际峰会或会议 |
| `joint_declaration` | 联合声明 | Joint Declaration | 与其他国家发表联合声明 |
| `public_opinion_guidance` | 舆论引导 | Public Opinion Guidance | 引导国际舆论 |

### 行为分类 (Behavior Categories)

系统还定义了**行为分类**，用于对行动进行更高层次的分类：

| 行为分类 | 包含行动类型 | 特征 |
|---------|----------|------|
| **合作类 (Cooperative)** | 外交访问、经济合作、经济援助、自由贸易、同盟升级 |
| **强制类 (Coercive)** | 军事行动、军事升级、经济制裁、贸易争端 |
| **规范类 (Normative)** | 规范提议、规范改革、组织改革、道德评价、争端仲裁 |
| **信息类 (Informational)** | 外交承认、峰会、联合声明、舆论引导 |
| **被动类 (Passive)** | 观察、延迟响应、不采取行动 |
...
├── main.tsx                       # 入口文件
├── components/                    # 可复用组件
│   ├── NormEvolutionChart.tsx   # 规范演化图表
│   ├── OrderTypeBadge.tsx       # 秩序类型徽章
│   └── SystemicEventsTable.tsx   # 体系事件表格
├── pages/                         # 页面组件
│   ├── Dashboard.tsx             # 仪表板首页
│   ├── SimulationMonitor.tsx       # 仿真监控页面
│   ├── AgentsManagement.tsx        # 代理管理页面
│   ├── Configuration.tsx           # 配置页面
│   └── SystemInteractionAnalysis.tsx # 体系交互分析页面
├── services/                      # API 服务
│   ├── api.ts                    # REST API 客户端
...
| 模块 | 功能描述 | 主要文件 |
|------|----------|----------|
| **仪表板首页** | 显示系统概览、仿真状态、指标汇总 | `Dashboard.tsx` |
| **仿真监控** | 实时控制仿真（启动/暂停/停止）、查看日志、进度条 | `SimulationMonitor.tsx` |
| **代理管理** | 查看、创建、编辑所有代理信息 | `AgentsManagement.tsx` |
| **配置页面** | 设置仿真参数、LLM 配置、环境变量 | `Configuration.tsx` |
| **体系分析** | 分析体系层面交互、规范演化、秩序类型 | `SystemInteractionAnalysis.tsx` |
| **规范图表** | 可视化国际规范随时间的演化 | `NormEvolutionChart.tsx` |
| **秩序徽章** | 显示当前国际秩序类型（多极/两极/单极等） | `OrderTypeBadge.tsx` |
| **事件表格** | 展示体系层面的重大事件 | `SystemicEventsTable.tsx` |
| **WebSocket 服务** | 实时接收仿真更新、推送日志 | `services/websocket.ts` |
| **状态管理** | 全局状态管理（仿真状态、代理列表、指标等） | `store/index.ts` |
...
| 因素 | 水平 |
|------|------|
| 自变量 1 | 领导类型 (4 水平: 道义型、霸权型、强权型、混合型) |
| 自变量 2 | 能力水平 (常量控制) |
