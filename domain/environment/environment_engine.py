...
class EnvironmentEngine:
    ...
    def trigger_random_events(
        ...
        if use_advanced_generator and agent_power_levels:
            # 使用高级事件生成器
            try:
                from domain.events.event_generator import EventGenerator
    ...
    def calculate_event_impact(
        ...
        """
        try:
            from domain.events.event_impact import EventImpactModel, PropagationModel
    ...
    def get_impact_history(self, limit: int = 10) -> List[Dict]:
        ...
        """
        try:
            from domain.events.event_impact import EventImpactModel
