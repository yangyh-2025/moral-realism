...
"""
    ...
    def build_shared_system(
        ...
        """
        Build the fully-shared system prompt. This is identical for ALL agents
        in the same round + phase, so it gets cached after the first agent call.
        ...
        if info_pool_dict:
            info_section = cls.INFO_POOL_TEMPLATE.format(
                ...
                alliance_chain_summary=info_pool_dict.get('alliance_chain_summary', '上一轮无重要联盟相关事件'),
                neighbor_summary=info_pool_dict.get('neighbor_summary', '【邻接关系简报】未提供邻接数据')
            )
        ...
        # Add task + JSON example (from original user prompt template)
        json_example = json.dumps(cls.JSON_OUTPUT_TEMPLATE, ensure_ascii=False, indent=2)
        task_section = f"""
【任务描述】
你需要作为{agent_name_for_task or '本国'}的国家领导集体，基于上述核心规则和全量信息池，做出符合国家利益的最优决策。
请从允许执行的行为列表中选择1-5项行为，并为每项行为提供详细的成本收益分析。

{chr(10) + '【当前态势摘要】' + chr(10) + situation_summary if situation_summary else ''}
...
"""
    ...
    def build_agent_user(
        ...
        """
        Build per-agent user prompt. Only contains agent-specific identity,
        rules, situation, and personal history perspective.
        All shared content is in the system prompt.
        """
